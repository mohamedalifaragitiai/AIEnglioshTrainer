...
# Attribution carried by every generated report. One constant rather than four
# strings, so the formats cannot drift into slightly different credits.
AUTHOR = "Abu Ali"
PRODUCT = "AI English Coach"
COPYRIGHT = f"© {{year}} {AUTHOR} · Built by {AUTHOR} · {PRODUCT}"


def copyright_line() -> str:
    """The credit, with the year resolved at render time rather than baked in."""
    from datetime import UTC, datetime

    return COPYRIGHT.format(year=datetime.now(UTC).year)
...
def render_csv(data: ReportData) -> bytes:
    ...
    writer.writerow([f"# {copyright_line()}"])
...
def render_xlsx(data: ReportData) -> bytes:
    ...
    # On the summary sheet, below the content: a workbook's first sheet is what
    # opens, and a credit nobody sees is not a credit. Also set as document
    # metadata, which survives copy-paste of the cells.
    ws.append([])
    ws.append([copyright_line()])
    wb.properties.creator = AUTHOR
    wb.properties.title = f"{PRODUCT} — {ov.display_name}"
...
def render_pdf(data: ReportData) -> bytes:
    ...
    c.setAuthor(AUTHOR)
    c.setTitle(f"{PRODUCT} — {ov.display_name}")
    c.setCreator(f"{PRODUCT} (built by {AUTHOR})")
    c.setFillColorRGB(*grey)
    c.setFont("Helvetica-Oblique", 8.5)
    ver = latest.scoring_model_version if latest else "v1"
    c.drawString(M, 1.2 * cm, f"Generated by {PRODUCT} · scoring {ver}")
    c.drawString(M, 0.8 * cm, copyright_line())
    c.showPage()
    c.save()
    return buf.getvalue()
